...
def save_raster(ref_im, prediction, name, n_clusters, mask=True):
    ...
    if mask:
        ...
        prediction.rio.to_raster(
            f'output/{name}_5month678910-kmeans-pca-{n_clusters}-0807.tiff',
            BIGTIFF="IF_SAFER",
            compress='LZW',
        ...
        # Save COG file to disk
        prediction.rio.to_raster(
            f'output/{name}_5month678910-pca-{n_clusters}-0807.tiff',
            BIGTIFF="IF_SAFER",
            compress='LZW',
            # num_threads='all_cpus',
            driver='GTiff',
...
def run():
    ...
    filename = f'{model_dir}kmeans-5month678910-{n_clusters}-tile02-southdakota-pca-0807.pkl'
