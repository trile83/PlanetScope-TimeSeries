...
def run():
    ...
    # save kmeans model
    model_dir = '/home/geoint/tri/Planet_khuong/output/spectral_model/'
    n_clusters = 7

    # Open a file and use dump()
    filename = f'{model_dir}spectralclus-ts-5month678910-{n_clusters}-{tile}-southdakota-0815.pkl'
    if os.path.isfile(filename):
        tic = time.time()
        print("Load model from file.")
        with open(filename, 'rb') as file:
            spectral_clust = pickle.load(file)
    else:
        tic = time.time()  
        # Run Spectral clustering
        print("Run Spectral Clustering from data.")

        print("number of clusters: ", n_clusters)
        
        spectral_clust = Birch(n_clusters=None)

        # tskmeans = TimeSeriesKMeans(n_clusters=n_clusters, metric="dtw", max_iter=10, random_state=42)
        
        if pca:
            spectral_clust.fit(pca_flat)
        else:
            spectral_clust.fit(flatImg)
            # tskmeans.fit(flatImg)

        with open(filename, 'wb') as file:
            pickle.dump(spectral_clust, file)
    ...
    if pca:
        prediction = spectral_clust.predict(pca_flat)
    else:
        prediction = spectral_clust.predict(full_flat)
